...
from django.contrib.auth.decorators import login_required#3.3.3关窗
...
def event_manage(request):
    ...
    # return  render(request,"event_manage.html",{"user":username})
    event_list = Event.objects.all()     # 5.2.1发布会列表，event_manege()函数定义
    username = request.session.get('username', '')   #导入Event类 + 通过Event.objects.all()查询所有数据
    return render(request, "event_manage.html", {"user": username,"events":event_list})
    #通过event_manage.html返回给客户端
...
def search_phone(request):
    ...
    guest_list = Guest.objects.filter(phone__contains=search_name_bytes).order_by('event_id')
