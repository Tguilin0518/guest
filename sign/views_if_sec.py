...
"""
第十一章：接口的安全机制 为接口参加安全机制：认证、签名、AES加密
"""
...
def user_auth(request):
    # user_auth函数的处理过程主要是提取用户认证数据并判断其正确性
    get_http_auth = request.META.get('HTTP_AUTHORIZATION', b'')
    #1) request.META是python的一个字典，包含了本次HTTP请求的Header信息：
    #       用户认证+ip地址+用户Agent(通常是浏览器的名称和版本号)等
    #2)HTTP_AUTHORIZATION 用于获取HTTP认证数据，空，则得到一个空的bytes对象
    auth = get_http_auth.split() #认证数据，通过splite方法拆分为list形式
    try:
        auth_parts = base64.b64decode(auth[1]).decode('utf-8').partition(':') # 加密
    except IndexError:
        return "null"
    userid, password = auth_parts[0], auth_parts[2] #解密
    user = django_auth.authenticate(username=userid, password=password) # django认证模块
    if user is not None and user.is_active:
        django_auth.login(request, user) # 在发布会接口--增加用户认证
        return "success"
    else:
        return "fail"
...
def get_event_list(request):
    auth_result = user_auth(request) # 调用认证函数 较弱的安全机制
    if auth_result == "null":
...
#=======用户签名+时间戳===============views_if_security

# 用户接口签名
def user_sign(request):

    if request.method == 'POST': # post的方法的获取两个参数
